...
to write arbitrary complex functions, at least in term of syntax. However, to
...
Hakellers are quite proud of the quicksort definition:
...
What is the time complexity of the above functions? (Except for 'fib'
...
Let's define an accumulator-style version of 'maximum'. The standard version is:
...
There's no need to limit ourselves to the 'Num' typeclass here, so let's give a
slightly more generic definition:
